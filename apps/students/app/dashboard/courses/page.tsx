...
export default function CoursesPage() {
  ...
  const { data, isLoading, error } = useQuery<CoursesPageQueryResult>({
    ...
    queryFn: () => graphqlClient.request<CoursesPageQueryResult>(COURSES_PAGE_QUERY, { studentId }),
    enabled: !!studentId,
  });

  const student = data?.student;
  const courses = useMemo(() => data?.studentCourses ?? [], [data?.studentCourses]);

  const levels = useMemo(() => {
    const numberOfYears = student?.department?.numberOfYears ?? 4;
    return Array.from({ length: numberOfYears }, (_, index) => (index + 1) * 100);
  ...
  useEffect(() => {
    ...
    const defaultLevel = levels.includes(currentLevel) ? currentLevel : levels[0];
  ...
  return (
    <main className="w-full px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 flex flex-wrap items-center gap-2">
        ...
        <Badge variant="outline">{student?.department?.name ?? 'Department not set'}</Badge>
      </div>
            ...
            <TabsContent key={level} value={String(level)}>
              ...
              >
                ...
                <TabsContent value="FIRST">
                  {activeSemester === 'FIRST' && Number(activeLevel) === level && (
                    <div className="space-y-3">
                      <p className="text-sm text-gray-600">{semesterLabel('FIRST')}</p>
                ...
                <TabsContent value="SECOND">
                  {activeSemester === 'SECOND' && Number(activeLevel) === level && (
                    <div className="space-y-3">
                      <p className="text-sm text-gray-600">{semesterLabel('SECOND')}</p>
                      {filteredCourses.length === 0 ? (
                        <div className="rounded-md border bg-gray-50 p-4 text-sm text-gray-600">
