...
function PaymentHistoryContent() {
  ...
  const { data: paymentsData, isLoading: paymentsLoading } = useQuery({
    ...
    queryFn: () => graphqlClient.request(PAYMENT_HISTORY_QUERY, { studentId }),
    enabled: !!studentId,
  });

  const { data: summaryData, isLoading: summaryLoading } = useQuery({
    queryKey: ["paymentSummary", studentId],
    queryFn: () => graphqlClient.request(PAYMENT_SUMMARY_QUERY, { studentId }),
    enabled: !!studentId,
  });
  ...
  if (isPending || paymentsLoading || summaryLoading) {
    return (
      <div className="text-center py-8 flex h-screen items-center justify-center">
