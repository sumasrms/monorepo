...
export function SessionSwitcher() {
  ...
  const activeSession = sessions.find(
    (s: AcademicSession) => s.id === currentSettings?.currentSessionId,
  );
            ...
            <CommandGroup heading="Active Sessions">
              {sessions.map((session: AcademicSession) => (
                <CommandItem
                  ...
                  onSelect={() => {
                    // Logic to switch session would go here
                    // If the user wants to ACTIVATE a session globally:
                    // activateSession.mutate({ sessionId: session.id, semester: "FIRST" });
                    // setOpen(false);
                  }}
