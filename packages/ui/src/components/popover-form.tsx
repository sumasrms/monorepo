...
export function PopoverForm({
  ...
  // Calculate popover position based on viewport edges
  useEffect(() => {
    if (!open || !triggerRef.current || !popoverRef.current) return;

    const calculatePosition = () => {
      const triggerRect = triggerRef.current!.getBoundingClientRect();
      const popoverWidth = parseInt(width) || 364;
      const popoverHeight = parseInt(height) || 192;
      const gap = 12; // spacing between trigger and popover
      ...
      let newPosition: PopoverPosition = "bottom";
      let top = 0;
      let left = 0;

      // Prefer bottom, but switch if not enough space
      ...
      if (left < 8) {
        ...
        left = viewport.width - popoverWidth - 8;
      }

      // Adjust for vertical overflow
      if (top < 8) {
        top = 8;
      } else if (top + popoverHeight > viewport.height - 8) {
        top = viewport.height - popoverHeight - 8;
      }
    ...
    <div>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            layoutId={`${title}-wrapper`}
            className="fixed p-1 overflow-hidden bg-muted shadow-[0_0_0_1px_rgba(0,0,0,0.08),0px_1px_2px_rgba(0,0,0,0.04)] outline-none z-50"
            ref={popoverRef}
            style={{
              borderRadius: 10,
              width,
